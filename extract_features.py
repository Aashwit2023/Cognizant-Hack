...
#         # --- UPDATED YDL OPTS TO BYPASS 403 FORBIDDEN ERROR ---
#         ydl_opts = {
#             'format': 'best[ext=mp4]',
#             'outtmpl': video_path,
#             'quiet': True,
...
#                 }
#             }
#             # Note: Agar phir bhi 403 aaye, toh niche wali line ko uncomment karke try karein:
...
class YouTubeFeatureExtractor:
    ...
    def extract_transcript(self, video_id):
        ...
        try:
            # 1. Fetch available transcripts directly using the Class
            transcript_list = YouTubeTranscriptApi.list_transcripts(video_id)
            
            # 2. Try getting English or Hindi first
            try:
                transcript = transcript_list.find_transcript(['en', 'hi']).fetch()
            except:
                # 3. Fallback: If no en/hi, fetch whatever is available
